...
def assign_initial(customers, vehicles, scenario):
    ...
    for customer_idx, vehicle_idx in mapping:
        api.update_scenario(scenario=scenario, customer=customers[customer_idx], vehicle=vehicle[vehicle_idx])
...
def assign_customers_to_vehicles_sequential(vehicles, customers):
    ...
    while unserved_customers:
        ...
        for customer_idx, vehicle_idx in mapping:
            vehicles[vehicle_idx].routePlan.add_customer(unserved_customers[customer_idx])
            unserved_customers[customer_idx] = None
        unserved_customers = list(filter(lambda a: a != None, unserved_customers))
        print(f"Unreserved: {unserved_customers}")
